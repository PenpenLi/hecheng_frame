...
@ccclass
export default class dlgMutantDragonOverdueItem extends cc.Component {
    /**玩家自己的框 */
    @property(cc.Sprite)
    ...
    public async loadingHead(container: cc.Sprite, remoteUrl: string) {
        ...
        if (remoteUrl === "") return;
        let png = await loader_mgr.ins().loadExternalAsset(remoteUrl, "png");
        container.spriteFrame = new cc.SpriteFrame(png)
    }
