...
export default class dataManager extends SingletonClass {
    ...
    token: string = window.dd_token;

    /***************固定参数↓↓********************* */
    Fixed_SuperTurnTable_Cost: number = 10;//超级转盘每次消耗10Mbc
    /***************固定参数↑↑********************* */

    FHBC: number = 220;
    //金币总数
    TotalCoins: BigVal = new BigVal("10");
    ...
    isHaveBird: Array<number> = [38, 38, 38, 38, 38, 38, 38, 38, 38, 38, 38, 38]; //传入数据
    //最高等级的
    ...
    /**当前屠龙刀数量 */
    dao_num: number = 80;
    dao_notice: string = "每1把“屠龙刀”增加2%的进度，\n进度条达到100%点击“屠龙刀”按钮即开启抽取。"
    /**限时分红的时间数组 */
    fenhong_breakerBird: Array<number> = [];
    superTurnTableAwards = []; //超级转盘所有数据
    video_type: number = 0;//视频类型 穿山甲or优量汇


    /**是否断开连接（403) */
    isCloseWebSocket: boolean = true;

    /**根据鸟的标记做实际等级的转换 */
    returnBirdLV(num: number) {
    ...
    getRandomNumVedio(num: number) {
        ...
        switch (num) {
            ...
            case 1:
                ...
                break;
            case 2:
                vedioType = this.NumberOfVideosLeft % 2;
                break;
        }
        return vedioType;
    }
     ...
     */
    useSuperTurnTable() {
        this.FHBC -= this.Fixed_SuperTurnTable_Cost;
    }
}
