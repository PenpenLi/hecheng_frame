...
import cfgManager from "../frame/script/game/CfgManager"
import BigVal from "../frame/script/common/bigval/BigVal"
import { uiManager } from "../frame/script/common/ui/uiManager";
import { uiFormType, UI_CONFIG_NAME, uiFormPath } from "../frame/script/common/config/gameConfigs";
...
export default class shopItem extends cc.Component {
    ...
    initOfThing(obj) {
        ...
        this.LabBirdsName.node.active = true;
        this.LabBirdsName.string = cfgManager.ins().getBirdName(this.IndexNum); //鸟的名字
        //两张图片
    ...
    showBtnNotBuy(obj) {
        ...
        this.LabBirdsName.node.active = false;
        this.BtncoinBuy.node.active = false;
        this.BtnJewelBuy.node.active = false;
    ...
    BtnOfcoinBuy() {
        if (dataManager.ins().is_Click_end) {
            ...
            cfgManager.ins().gameManager.buyPet(1, this.IndexNum, call);
        } else {
            console.log("请勿快速点击");
        }
    }
    ...
    BtnOfjewelBuy() {
        if (dataManager.ins().FHBC < this.costOfjewel) {
            ...
            if (dataManager.ins().is_Click_end) { //防止快速点击
                ...
                cfgManager.ins().gameManager.buyPet(2, this.IndexNum)
            } else {
                console.log("请勿快速点击");
            }
        }
