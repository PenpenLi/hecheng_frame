...
export default class NewClass extends cc.Component {
    ...
    sendMessageFhbc() {
        ...
        var funErr = function (ret) {
            if (ret.data.msg) {
                // console.log("ret", ret)
                cfgManager.ins().gameManager.gameTips(ret.data.msg);
            }
        }
        let params = {
            id: self.id
        }
        cfgManager.ins().http.sendRequest('/api/game/receiveFhbc', params, funSuc, funErr);
    }
    ...
    jewelMove(self) {
        ...
        var action = cc.sequence(
            ...
            cc.callFunc(function () {
                cfgManager.ins().Tops.ClickJewelEnd(price); //加金币
                self.node.destroy();
            }, self),
        )
