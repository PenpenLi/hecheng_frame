...
export default class DlgOffLine extends baseUi {
    ...
    /**初始化值 */
    initOffLineCoin() {
        let price = userData.ins().offLineCoin;
        ...
        if (userData.ins().NumberOfVideosLeft > 0) { //剩余视频次数
            ...
            this.Btnlookvideo.getChildByName("cishu").getComponent(cc.Label).string = userData.ins().strOfLookVideo();
        } else {
    ...
    BtnWatchVideo() {
        ...
        this.btnClose();
        let price_v = BigVal.Mul(userData.ins().offLineCoin, new BigVal("2"));
    ...
    Btnuserquano() {
        ...
        var call = function () {
            let price_q = BigVal.Mul(userData.ins().offLineCoin, new BigVal("2"));
